...
public class Player extends Entity {
	...
	private int powerMaxValue = 120;
	private float powerValue = powerMaxValue;
	...
	private void updateAttackBox() {
		if (right && left) {
			if (flipW == 1) {
				attackBox.x = hitbox.x + hitbox.width + hitbox.width + (int) (Game.SCALE * 5);
			} else {
				attackBox.x = hitbox.x - hitbox.width - hitbox.width - (int) (Game.SCALE * 2);
			}
	...
	private void updateAnimationTick() {
		aniTick++;
		if (aniTick >= ANI_SPEED||(attacking==true&&aniTick>=ANI_SPEED_PLAYER_ATTACK)) {
			aniTick = 0;
			aniIndex++;
