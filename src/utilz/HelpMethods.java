...
import static utilz.Constants.EnemyConstants.SKELETON_MAGE;
...
public class HelpMethods {
	...
	public static ArrayList<Skeleton_Mage> GetSkeletons_Mage(BufferedImage img) {
		ArrayList<Skeleton_Mage> list = new ArrayList<>();
		for (int j = 0; j < img.getHeight(); j++)
			for (int i = 0; i < img.getWidth(); i++) {
				Color color = new Color(img.getRGB(i, j));
				int value = color.getGreen();
				if (value == SKELETON_MAGE){
					list.add(new Skeleton_Mage(i * Game.TILES_SIZE, j * Game.TILES_SIZE));
